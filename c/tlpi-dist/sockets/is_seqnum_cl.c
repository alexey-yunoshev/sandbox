...
main(int argc, char *argv[])
{
    ...
    getaddrinfo(argv[1], PORT_NUM, &hints, &first_addr);
    ...
    char *message = (argc < 2) ? argv[2] : "1";
    write(client_fd, message, strlen(message));
    write(client_fd, "\n", 1);

    char response[INT_LEN];
    readLine(client_fd, response, INT_LEN);
    printf("Sequence number: %s", response);
    exit(EXIT_SUCCESS);
}